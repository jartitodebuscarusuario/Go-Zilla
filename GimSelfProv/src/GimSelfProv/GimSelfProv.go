...
func initinf(infid string, confvars map[string]int) {
	...
	confvar := "upcpu"
	wprint("confvar:",confvar, ": ", tmpconf.(map[string]int)[confvar])
...
func main() {
	...
	f, err := os.Create("C:\\Users\\Ariadnaya\\Documents\\tmp\\perl\\GoReport_server.txt")
	...
	mapcmap.Set("foo",confvars)
	if tmp, ok := mapcmap.Get("foo"); ok {
		bar := tmp.(map[string]int)
		wprint("foo upcpu: ",bar["upcpu"])
	}
